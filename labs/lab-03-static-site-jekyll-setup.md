...
## Set Up Jekyll and Develop Your Basic Site
...
1. Create a new folder/directory on your local computer. This will be the location you use for building and developing your Jekyll site. It's likely you may keep this around long term, so give it a name like `jekyll-test-site-1` (or something similarly generic).
...
Initiate a new GitHub repository, connect it to a remote repo on GitHub, and initate the publication workflow with GitHub Actions.
...
2. Next, connect your repo to a remote repo on GitHub. For ease of management, I suggest naming this repo the same thing as the folder where your Jekyll template site was created. Connect the local to the remote (use a command like `git add origin <URL to your GitHub repo>`). Then, push the files (run `git push -u origin main`).
3. Activate GitHub pages and a GitHub Actions workflow for your remote repo (in most cases, you can use the default `.yml` configuration file that GitHub suggests). As noted in the static site guide, there are a few additional modifications to the Gemfile and `_config.yml` to get things ready to publish.
4. Finally, publish your site via GitHub.
